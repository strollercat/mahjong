 ...
 */
@Controller
public class GetTotalRoomsByRecommmendOfManagerController {

	private static final Logger logger = LoggerFactory
			.getLogger(GetTotalRoomsByRecommmendOfManagerController.class);
	...
	@RequestMapping(value = "getTotalRoomsByRecommmendOfManager")
	@ResponseBody
	public Response getTotalRoomsByRecommmend(HttpServletRequest request,
			String date, int agentId) {
		...
		if (agentGu == null) {
			return ResponseFactory.newResponse("000002", "不存在ID为" + agentId
					+ "的代理商");
		}
		if (!agentGu.isRecommend()) {
			return ResponseFactory.newResponse("000003", "不存在ID为" + agentId
					+ "的代理商");
		}
		...
		Map mapRet = gameStaticsService
				.getTotalRoomsAndMoneyConsumedByRecommmendAndTimeRange(agentId,
						timeRange);
