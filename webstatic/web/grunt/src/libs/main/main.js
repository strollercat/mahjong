...
//		alert('host:'+host);
		...
		if(msg.roomInfo.name == 'ningboMajiang'){
			var title ='宁波麻将';
			document.title=title;
			title += (msg.roomInfo.baida + '百搭-'+msg.roomInfo.totalJu+'局-'+msg.roomInfo.playerNum+'人-房间号'+msg.roomId);
		...
		}else if(msg.roomInfo.name == 'fenhuaMajiang'){
			var title ='奉化麻将';
			document.title=title;
			title += '-';
			title += ((msg.roomInfo.pinghu?"平搓":"冲刺")+'-'+msg.roomInfo.totalJu+'局-'+msg.roomInfo.playerNum+'人-房间号'+msg.roomId);
			game.shareData.title = title;
		}else if(msg.roomInfo.name == 'tiantaiMajiang'){
			var title ='天台麻将';
			document.title=title;
			title += '-';
			title += (msg.roomInfo.totalJu+'局-'+msg.roomInfo.playerNum+'人-房间号'+msg.roomId);
			game.shareData.title = title;
		}else if(msg.roomInfo.name == 'hongzhongMajiang'){
			var title ='红中麻将';
			document.title=title;
			title += '-';
			title += (msg.roomInfo.totalJu+'局-底分'+msg.roomInfo.diScore+'-'+msg.roomInfo.playerNum+'人-房间号'+msg.roomId);
			game.shareData.title = title;
			
		}else if(msg.roomInfo.name == 'guangdongMajiang'){
			var title ='广东麻将';
			document.title=title;
			title += '-';
			title += (msg.roomInfo.totalJu+'局-底分'+msg.roomInfo.diScore+'-'+msg.roomInfo.playerNum+'人-房间号'+msg.roomId);
		...
		}else if(msg.roomInfo.name == 'hangzhouMajiang'){
			var title ='杭州麻将';
			document.title=title;
			title += '-';
			title += (msg.roomInfo.totalJu+'局-'+msg.roomInfo.playerNum+'人-房间号'+msg.roomId);
			...
			var desc = msg.roomInfo.baiBanBaida?'白板百搭 ':'翻财神 ';
			desc+=(msg.roomInfo.sanLao?'笃三老庄 ':'平庄起 ');
			desc+=(msg.roomInfo.sanTan?'三滩承包(庄闲) ':'不可吃三滩 ');
			desc+=(msg.roomInfo.zimoHu?'自摸胡 ':'庄闲放冲 ');
			desc+=(msg.roomInfo.baidaiKaoxiang?'有财神必拷响 ':'');
			desc+=(msg.roomInfo.pengTan?'碰算摊 ':'');
			desc+=(msg.roomInfo.baidaHaoqi?'财神替豪七杠子 ':'');
			game.shareData.desc = desc;
		}else if(msg.roomInfo.name == 'xiangshanMajiang'){
			var title ='象山麻将';
			document.title=title;
			title += '-';
			title += (msg.roomInfo.totalJu+'局-'+msg.roomInfo.playerNum+'人-房间号'+msg.roomId);
